...
const ListVideoTemplate = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `https://databaseswap.mangasocial.online/get/list_video/all_video_wedding_template`
      )
      .then((res) => {
            ...
            <div className="">
              <h1 className="font-[600] text-[40px] leading-[48px] text-center mb-[10px] mt-[10px]">
                {valueLocation.geoplugin_city === "Hanoi"
                  ? "Mẫu video có sẵn"
                  : "Wedding Template Video"}
              </h1>
              <h5 className="font-[400] italic md:text-[20px] text-[16px] leading-[20px] text-center mb-[30px] mt-[10px]">
                {valueLocation.geoplugin_city === "Hanoi"
                  ? "Chọn mẫu phù hợp với bạn để bắt đầu tạo những video chỉ có một lần trong đời này"
                  : "Choose a template that's right for you to start creating these once-in-a-lifetime videos"}
              </h5>
              <div className="">
                  ...
                  {listTemp.slice(0, 100).map((image, index) => {
                    return (
                      <div className="mt-4 md:mt-0" key={index}>
                        ...
                        <Link to={`/uploadvideo/${image.id}`}>
                          <Button
                            variant={"outline"}
                            className="md:ml-[80px] ml-[50px] mt-3"
                          >
                            Start
                          </Button>
                        </Link>
