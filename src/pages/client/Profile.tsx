...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://databaseswap.mangasocial.online/get/list_2_image/id_image_swap?id_user=${id}`
      )
  ...
  useEffect(() => {
    axios
      .get(
        `https://databaseswap.mangasocial.online/get/list_video_wedding/id_video_swap?id_user=${id}`
      )
      .then((res) => {
        ...
        <div className="fixed inset-0 z-50 bg-black/50">
          <div className="cursor-pointer" onClick={() => setIsOpen(false)}>
            <svg
              ...
              strokeWidth="1.5"
              stroke="white"
              className="w-10 h-10 float-right mr-[30px] mt-[30px]"
            >
              <path
                strokeLinecap="round"
          ...
          <div className="absolute top-[50%] left-[50%] z-50 md:w-[50%] w-[70%] translate-x-[-50%] translate-y-[-50%] gap-4 border md:p-6 p-2 shadow-lg rounded-2xl items-center justify-center text-center bg-white opacity-100">
            <Carousel>
              <div className="md:py-[30px]">
                <CarouselContent>
                  {albumData &&
                    albumData[currentAlbum]?.map(
                      (item: { link_da_swap: string }, index: number) => {
                        return (
                          <>
        ...
        <div className="pb-[70px] pt-[40px]">
          <Link to={"/"} className="">
            <svg
              ...
              strokeWidth="1.5"
              stroke="#808080"
              className="w-10 h-10 float-right mr-[40px]"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
          ...
          <h1 className="font-[600] text-[40px] leading-[48px] md:ml-[210px] ml-7 mb-[30px] mt-[10px]">
            Profile
          </h1>
          <div className="mx-auto p-[24px] md:w-[1020px] rounded-xl border bg-white border-gray-100 shadow-xl hover:border-gray-200 hover:ring-1 hover:ring-gray-200 focus:outline-none focus:ring md:flex gap-8">
            <div className="md:w-[340px] w-[390px]">
              <div className="bg-white border-l-[16px] rounded-xl border-[#16B6D4] md:pl-11 pl-2 flex md:block">
                ...
                <div className="text-[16px] md:text-[18px] font-[400] leading-[24px] mt-2 ml-2">
                  <p className="text-[24px] font-[700] leading-[24px] mt-2">
                    {user.user_name}
                  </p>
              ...
              {currentUser.id_user == id && (
                <Button
                  onClick={() => navi(`/profile/edit/${currentUser.id_user}`)}
                  variant={"cus2"}
                  className="mx-auto"
                >
                  {valueLocation.geoplugin_city === "Hanoi"
            ...
            <div className="md:w-full">
              <h3 className="font-[600] text-[24px] leading-[48px] ml-[10px] mb-[30px] mt-[10px] italic">
                {valueLocation.geoplugin_city === "Hanoi"
                  ? "Album ảnh cưới"
                  : "Wedding album"}
              </h3>
                  ...
                  return (
                    ...
                    >
                      <div className="md:w-[300px] md:h-[450px] w-[150px] h-[225px] hover:opacity-70 rounded-2xl overflow-hidden">
                        <img
                          src={image[0].link_da_swap}
                          className="h-full w-full object-cover"
                          alt={`Image ${index}`}
                        />
                      </div>
                      <div className="absolute">
                        <Link to={`/timeline/event/${image[0].id_sk_album}`}>
                          <Button
                            variant={"default"}
                        ...
                        <Button
                          variant={"outline"}
                          className="transition-all transform translate-y-8 opacity-0 group-hover:opacity-100 group-hover:translate-y-0 w-[160px] mt-3 rounded-3xl"
                  ...
                  return (
                    ...
                    >
                      <div className="md:w-[300px] md:h-[450px] w-[150px] h-[225px] hover:opacity-90 rounded-2xl overflow-hidden">
                        <video
                          src={video.link_video_da_swap}
                          className="h-full w-full object-cover"
                          controls
                        />
                      </div>
                      <div className="absolute">
                        <Link to={`/funnyvideo/detail/${id}/${video.id_saved}`}>
                          <Button
                            variant={"default"}
                            className="transition-all transform translate-y-8 opacity-0 group-hover:opacity-100 
                                                group-hover:translate-y-0 w-[160px] rounded-3xl"
