...
const Editor = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://databaseswap.mangasocial.online/images/${id}?type=video`, {
        headers: {
  ...
  const handleSaveForm = () => {
    if (avatarUploadFile) {
      ...
      axios
        .post(
          `https://databaseswap.mangasocial.online/upload-gensk/${user.id_user}?type=src_vid`,
          formData,
          {
            headers: {
  ...
  return (
    <div className="p-4 w-[390px] md:p-0 md:w-fit">
      <h3 className="font-[600] md:text-[20px] leading-[32px] text-[18px]">
        Profile information
      </h3>
      <Form {...form}>
        <form action="" onSubmit={form.handleSubmit(handleSaveForm)}>
          <div className="flex my-auto md:mt-[45px] mt-[24px]">
            <input {...getUploaderInputProps()} className="w-full h-full" />
            <div
              className="w-[96px] h-[96px] rounded-[48px] overflow-hidden"
            ...
            <button
              ...
              onClick={() => {
                openUploader();
                setChooseUploaded(false);
              }}
              className="text-[#fff] w-[253px] bg-[#16B6D4] h-[54px] my-auto md:ml-8 ml-2 rounded-[27px] px-[20px] py-[10px] text-center font-[700] text-[16px] leading-[24px] flex mx-auto items-center"
            >
          ...
          <h3 className="font-[600] text-[16px] leading-[20px] mt-10 hidden md:block">
            {valueLocation.geoplugin_city === "Hanoi"
              ? "Đã tải lên"
              : "Uploaded"}
          </h3>
          <div className="md:grid grid-cols-4 gap-8 w-[638px] rounded-[20px] hidden mt-6">
            {seeMore
              ? uploadedImage.map((image, index) => {
                  return (
                    <img
                      onClick={() => handleChooseUploaded(image)}
                      src={image}
                      className="object-cover w-[136px] h-[136px]"
                      alt={`Image ${index}`}
                      key={index}
                    />
                  );
          ...
          <button
            ...
            className="text-[#fff] hidden md:block w-[638px] bg-[#16B6D4] h-[54px] my-auto mt-4 rounded-[27px] px-[20px] py-[10px] text-center font-[700] text-[16px] leading-[20px] mx-auto items-center"
          >
          ...
          />
          <Button variant={"cus1"} type="submit" className="mt-11">
            {valueLocation.geoplugin_city === "Hanoi" ? "Lưu" : "Save"}
          </Button>
        </form>
      </Form>
    </div>
